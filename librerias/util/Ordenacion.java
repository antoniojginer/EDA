...
public class Ordenacion {
    ...
    */
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
        /*COMPLETAR*/
        int a = v1.length, b = v2.length, i = 0, k = 0;
        T[] aux = (T[]) new Comparable[a+b];
        for(; i<a && k < b;i++,k++){
            if(v1[i].compareTo(v2[k])<0){ aux[i+k] = v1[i++];}
            else{ aux[i+k] = v2[k++];}
        }
        
        while(i < a){ aux[i+k] = v1[i++]; }
        while(k < b){ aux[i+k] = v2[k++]; }
