...
public class TablaHash<C, V> implements Map<C, V> {
    ...
    */
    ...
    public ListaConPI<C> claves() {
        ...
        int tamaño = elArray.length;
        for(int i = 0; i<tamaño; i++){
            if(elArray[i] != null){
                for(elArray[i].inicio(); !elArray[i].esFin(); elArray[i].siguiente()){
                    claves.insertar(elArray[i].recuperar().clave);
                }
            }
        }
    ...
    protected final void rehashing() {
        ...
        int tamaño = siguientePrimo(elArray.length * 2);
        ListaConPI<EntradaHash<C,V>> [] res = elArray;
        elArray = new LEGListaConPI[tamaño];
        for(int i = 0; i<elArray.length; i++){
            elArray[i] = new LEGListaConPI<>();
        }
        
        for(int i=0; i<res.length; i++){
            for(res[i].inicio(); !res[i].esFin(); res[i].siguiente()){
                EntradaHash<C,V> elemento = res[i].recuperar();
    ...
    public final double desviacionTipica() {
        //COMPLETAR
        double media = talla/elArray.length;
        double sumatorio = 0;
        for(int i = 0; i<elArray.length; i++){
            sumatorio += (elArray[i].talla()-media) * (elArray[i].talla()-media);
    ...
    public String histograma() {
        ...
        for(int i = 0; i< elArray.length; i++){
            if(elArray[i].talla() >= 9){
                hist[9]++;
            } else{
                hist[elArray[i].talla()]++;
            }
        }
